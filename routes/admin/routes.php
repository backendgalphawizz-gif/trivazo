...
use App\Http\Controllers\Admin\RoomManagement\RoomController;
...
Route::group(['prefix' => 'admin', 'as' => 'admin.', 'middleware' => ['admin']], function () {
    Route::group(['prefix' => 'dashboard', 'as' => 'dashboard.'], function () {
        Route::controller(DashboardController::class)->group(function () {
             ...
             Route::get('food', 'foodDashboard')->name('food');
            Route::get('tow', 'towDashboard')->name('tow');
...
|--------------------------------------------------------------------------
*/
Route::group(['prefix' => 'hotels', 'as' => 'hotels.', 'middleware' => ['module:hotel_management']], function () {
    ...
    Route::get('pending', [HotelController::class, 'pending'])->name('pending');
    Route::get('approved', [HotelController::class, 'approved'])->name('approved');
    Route::get('rejected', [HotelController::class, 'rejected'])->name('rejected');
    
    // View single hotel
    Route::get('view/{id}', [HotelController::class, 'view'])->name('view');
    
    // Approve/Reject actions
    Route::post('approve/{id}', [HotelController::class, 'approve'])->name('approve');
    Route::post('reject/{id}', [HotelController::class, 'reject'])->name('reject');
    
    // Update status (featured, etc.)
    Route::post('status/{id}', [HotelController::class, 'updateStatus'])->name('status');
    
    // Export hotels
    Route::get('export/{type}', [HotelController::class, 'export'])->name('export');
    
    // Delete hotel
    Route::delete('delete/{id}', [HotelController::class, 'delete'])->name('delete');

    // hotel services routes
    Route::get('services', [HotelController::class, 'servicesIndex'])->name('services-index');
    Route::get('{hotel_id}/services', [HotelController::class, 'services'])->name('services');
    Route::post('{hotel_id}/services', [HotelController::class, 'addService'])->name('add-service');
    Route::get('edit-service/{id}', [HotelController::class, 'editService'])->name('edit-service');
    Route::put('update-service/{id}', [HotelController::class, 'updateService'])->name('update-service');
    Route::post('service-status/{id}', [HotelController::class, 'updateServiceStatus'])->name('service-status');
    Route::delete('delete-service/{id}', [HotelController::class, 'deleteService'])->name('delete-service');

    // amenties routes (rahul 03-03-2026)
    Route::get('all-amenities', [HotelController::class, 'amenities'])->name('all-amenities');
    Route::post('add-amenity', [HotelController::class, 'addAmenities'])->name('add-amenity');
    Route::get('edit-amenity/{id}', [HotelController::class, 'editAmenities'])->name('edit-amenity');
    Route::put('update-amenity/{id}', [HotelController::class, 'updateAmenities'])->name('update-amenity');
    Route::delete('delete-amenity', [HotelController::class, 'deleteAmenities'])->name('delete-amenity');
...
/*
|--------------------------------------------------------------------------
| Tow Management Routes
|--------------------------------------------------------------------------
*/
Route::group(['prefix' => 'tow-management', 'as' => 'tow-management.', 'middleware' => ['module:tow_management']], function () {
    
    // Tow Providers Routes
    Route::group(['prefix' => 'providers', 'as' => 'providers.'], function () {
        Route::controller(TowProviderController::class)->group(function () {
            // List/Index
            ...
            // Trips History
            Route::get('trips/{id}', 'getProviderTrips')->name('trips');
            
            // Status Updates (AJAX)
            Route::post('status', 'updateStatus')->name('status');
            Route::post('availability', 'updateAvailability')->name('availability');
    ...
    Route::group(['prefix' => 'requests', 'as' => 'requests.'], function () {
        Route::controller(TowRequestController::class)->group(function () {
            // List/Index
            Route::get('list', 'getListView')->name('list');
    ...
    // Active Trips Routes
    Route::group(['prefix' => 'active-trips', 'as' => 'active-trips.'], function () {
        Route::controller(ActiveTripController::class)->group(function () {
            // List/Index
            Route::get('list', 'getListView')->name('list');
            
            // Map View
            Route::get('map-view', 'getMapView')->name('map-view');
            
            // Details View
            Route::get('details/{id}', 'getDetailsView')->name('details');
            
            // Live Tracking
            Route::get('live-tracking/{id}', 'getLiveTrackingView')->name('live-tracking');
            
            // Assign Provider
            Route::post('assign', 'assignProvider')->name('assign');
            
            // Reassign Provider
            Route::post('reassign', 'reassignProvider')->name('reassign');
            
            // Status Update (AJAX)
            Route::post('update-status', 'updateTripStatus')->name('update-status');
            
            // Live Tracking Data (AJAX)
            Route::get('get-tracking-data', 'getLiveTrackingData')->name('get-tracking-data');
        });
    });
});
...
Route::group([
    'prefix' => 'rooms',
    'as' => 'rooms.',
    'middleware' => ['module:room_management']
], function () {
    ...
    // View single room
    Route::get('view/{id}', [RoomController::class, 'view'])->name('view');

    // Update status
    Route::post('status/{id}', [RoomController::class, 'updateStatus'])->name('status');
...
/*
|--------------------------------------------------------------------------
| Hotel Bookings Routes (Monitor bookings & cancellations)
|--------------------------------------------------------------------------
*/
Route::group(['prefix' => 'hotel-bookings', 'as' => 'hotel-bookings.', 'middleware' => ['module:hotel_management']], function () {
    
    // Monitor all bookings
    Route::get('/', [HotelBookingController::class, 'index'])->name('index');
    Route::get('pending', [HotelBookingController::class, 'pending'])->name('pending');
    Route::get('confirmed', [HotelBookingController::class, 'confirmed'])->name('confirmed');
    Route::get('cancelled', [HotelBookingController::class, 'cancelled'])->name('cancelled');
    Route::get('completed', [HotelBookingController::class, 'completed'])->name('completed');
    
    // View single booking
    Route::get('view/{id}', [HotelBookingController::class, 'view'])->name('view');
    
    // Cancel booking (if required)
    Route::post('cancel/{id}', [HotelBookingController::class, 'cancel'])->name('cancel');
    
    // Update booking status
    Route::post('update-status/{id}', [HotelBookingController::class, 'updateStatus'])->name('update-status');
...
/*
|--------------------------------------------------------------------------
| Room Inventory Routes (Manage room inventory visibility)
|--------------------------------------------------------------------------
*/
Route::group(['prefix' => 'room-inventory', 'as' => 'room-inventory.', 'middleware' => ['module:hotel_management']], function () {
    
    // Manage inventory visibility
    Route::get('/', [RoomInventoryController::class, 'index'])->name('index');
    
    // Update single room visibility
    Route::post('update-visibility/{id}', [RoomInventoryController::class, 'updateVisibility'])->name('update-visibility');
    
    // Calendar view
    Route::get('calendar/{hotel_id}', [RoomInventoryController::class, 'calendar'])->name('calendar');
    
    // Bulk update inventory
    Route::post('bulk-update', [RoomInventoryController::class, 'bulkUpdate'])->name('bulk-update');
});
