...
function syncFoodAddonSelections(sourceFormSelector = ".add-to-cart-details-form", targetFormSelector = ".add-to-cart-sticky-form") {
    let sourceForm = $(sourceFormSelector);
    ...
    let mirrorContainer = targetForm.find("[data-food-addon-mirror]");
    if (!mirrorContainer.length) {
        return;
    }

    mirrorContainer.empty();
    sourceForm.find('input[name^="selected_food_addons["]:checked').each(function () {
        $('<input>', {
            type: 'hidden',
            name: $(this).attr('name'),
            value: $(this).val(),
...
function initializeFoodAddonSelection() {
    $(document).off('change.foodAddonSelection').on('change.foodAddonSelection', '.food-addon-input', function () {
        let currentInput = $(this);
        let currentGroup = currentInput.closest('.food-addon-group');
        let maxSelect = parseInt(currentGroup.data('max-select'), 10) || 1;

        if (currentInput.is(':checkbox') && currentGroup.find('.food-addon-input:checked').length > maxSelect) {
            currentInput.prop('checked', false);
            toastr.warning(`You can select up to ${maxSelect} add-ons in this group.`);
            return;
        }

        syncFoodAddonSelections();
        if (currentInput.closest('.add-to-cart-details-form').length) {
            getVariantPrice('.add-to-cart-sticky-form');
        }
    });
}

initializeFoodAddonSelection();
