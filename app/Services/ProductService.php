...
class ProductService
{
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        $foodProductData = $this->getFoodProductData(request: $request);
        ...
        return [
            ...
            'is_food' => $foodProductData['is_food'],
            'food_type' => $foodProductData['food_type'],
            'prep_time' => $foodProductData['prep_time'],
            'available_from' => $foodProductData['available_from'],
            'available_to' => $foodProductData['available_to'],
            'food_addons' => $foodProductData['food_addons'],
    ...
    private function getFoodProductData(object $request): array
    {
        $isFood = $request['product_type'] == 'physical' && $request->boolean('is_food');

        return [
            'is_food' => $isFood,
            'food_type' => $isFood ? $request->input('food_type') : null,
            'prep_time' => $isFood && $request->filled('prep_time') ? (int) $request->input('prep_time') : null,
            'available_from' => $isFood ? $request->input('available_from') : null,
            'available_to' => $isFood ? $request->input('available_to') : null,
            'food_addons' => $isFood ? $this->normalizeFoodAddons($request->input('food_addons', [])) : [],
        ];
    }

    private function normalizeFoodAddons(array $foodAddons): array
    {
        $normalizedAddons = [];

        foreach ($foodAddons as $addon) {
            $addonName = trim((string) data_get($addon, 'name', ''));
            $options = [];

            foreach ((array) data_get($addon, 'options', []) as $option) {
                $optionName = trim((string) data_get($option, 'name', ''));
                if ($optionName === '') {
                    continue;
                }
                ...
                $options[] = [
                    'name' => $optionName,
                    'price' => round((float) data_get($option, 'price', 0), 2),
                ];
            }

            if ($addonName === '' || empty($options)) {
                continue;
            }

            $minSelect = max(0, (int) data_get($addon, 'min_select', 0));
            $maxSelectInput = (int) data_get($addon, 'max_select', count($options));
            $maxSelect = $maxSelectInput > 0 ? min($maxSelectInput, count($options)) : count($options);
            $minSelect = min($minSelect, $maxSelect);

            $normalizedAddons[] = [
                'name' => $addonName,
                'is_required' => (bool) data_get($addon, 'is_required', false),
        ...
        return array_values($normalizedAddons);
    }
