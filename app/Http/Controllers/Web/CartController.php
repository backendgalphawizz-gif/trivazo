...
class CartController extends Controller
{
    ...
    public function getVariantPrice(Request $request): array
    {
        ...
        $foodAddonData = CartManager::getSelectedFoodAddonData(product: $product, request: $request);
        ...
        foreach ($getCartList as $cartItem) {
            if ($cartItem['product_id'] == $product['id'] && $cartItem['variant'] == $string && ($cartItem['food_addons'] ?? []) == $foodAddonData['food_addons']) {
                $inCartExistStatus = 1;
        ...
        $addonTotal = (float)($foodAddonData['food_addon_total'] ?? 0);
        $price += $addonTotal;
        $discountedUnitPrice += $addonTotal;
