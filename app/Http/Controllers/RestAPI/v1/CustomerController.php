...
class CustomerController extends Controller
{
    ...
    public function update_profile(Request $request)
    {
        ...
        $countryCode = $request->filled('country_code') ? $request->country_code : $user->country_code;

        if ($email && User::where('email', $email)->where('id', '!=', $user->id)->exists()) {
            return response()->json(['errors' => [[
                'code' => 'email',
                'message' => translate('this_email_has_already_been_used_in_another_account!')
            ]]], 403);
        }

        if ($phone && User::where('phone', $phone)
            ->where('country_code', $countryCode)
            ->where('id', '!=', $user->id)
            ->exists()) {
            return response()->json(['errors' => [[
                'code' => 'phone',
                'message' => translate('this_phone_has_already_been_used_in_another_account!')
            ]]], 403);
        }
        ...
        $fullName = trim((string) ($request->name ?? $user->name));
        $nameParts = preg_split('/\s+/', $fullName, 2);
        $firstName = $nameParts[0] ?? ($user->f_name ?? '');
        $lastName = $nameParts[1] ?? $user->l_name;
        $userDetails = [
            'name' => $fullName !== '' ? $fullName : $user->name,
            'f_name' => $firstName,
            'l_name' => $lastName !== '' ? $lastName : null,
            'image' => $imageName,
            'phone' => $phone,
            'country_code' => $countryCode,
            'email' => $email,
            'is_phone_verified' => $phone == $user['phone'] ? $user['is_phone_verified'] : 0,
            'is_email_verified' => $email == $user['email'] ? $user['is_email_verified'] : 0,
            ...
            'currency' => $request->filled('currency') ? $request['currency'] : $user->currency,
            'updated_at' => now(),
        ];
