...
class ProductAddRequest extends Request
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'food_type' => 'required_if:is_food,on|nullable|in:veg,non_veg,egg,jain,beverage',
            'prep_time' => 'required_if:is_food,on|nullable|integer|min:1|max:1440',
    ...
    public function after(): array
    {
        return [
            function (Validator $validator) {
                ...
                if ($this->boolean('is_food')) {
                    $foodAddons = $this->input('food_addons', []);
                    foreach ($foodAddons as $groupIndex => $addon) {
                        $addonName = trim((string) ($addon['name'] ?? ''));
                        $options = array_filter((array) ($addon['options'] ?? []), function ($option) {
                        ...
                        if ($addonName === '') {
                            $validator->errors()->add('food_addons', 'Addon group name is required.');
                        }

                        if (empty($options)) {
                            $validator->errors()->add('food_addons', 'Each addon group must have at least one option.');
                        }

                        $minSelect = (int) ($addon['min_select'] ?? 0);
                        $maxSelect = (int) ($addon['max_select'] ?? 0);
                        if ($maxSelect > 0 && $minSelect > $maxSelect) {
                            $validator->errors()->add('food_addons', 'Addon minimum selection cannot exceed maximum selection.');
                        }

                        foreach ($options as $optionIndex => $option) {
                            if (!is_numeric($option['price'] ?? 0) || (float) ($option['price'] ?? 0) < 0) {
                                $validator->errors()->add('food_addons', 'Addon option price must be zero or positive.');
                            }
                        }
                    }
                }
