...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        foreach (CartManager::getCartListQuery(groupId: $data['cart_group_id'], type: 'checked') as $cartSingleItem) {
            ...
            $productDiscount = $cartSingleItem['discount'];
            $orderDetails = [
                'order_id' => $order_id,
                ...
                'food_addons' => $cartSingleItem['food_addons'] ?? json_encode([]),
                'food_addon_total' => $cartSingleItem['food_addon_total'] ?? 0,
    ...
    public static function order_insert($order_data): int
    {
        ...
        foreach (CartManager::getCartListQuery(groupId: $order_data['data']['cart_group_id']) as $c) {
            ...
            $orderDetails = [
                ...
                'food_addons' => $c['food_addons'] ?? json_encode([]),
                'food_addon_total' => $c['food_addon_total'] ?? 0,
                'delivery_status' => 'pending',
                'shipping_method_id' => null,
                'payment_status' => 'unpaid',
                'created_at' => now(),
            ...
            if ($product) {
                ...
                if ($productValid) {
                    ...
                    if ($productValid && $price != 0) {
                        $price += (float)($orderProduct->food_addon_total ?? 0);
                        $cartExist = Cart::where([
                            'customer_id' => $user->id,
                            'variations' => $orderProduct->variation,
                            'product_id' => $orderProduct->product_id,
                            'food_addons' => $orderProduct->food_addons ?? json_encode([]),
                        ])->first();
                        ...
                        if (!$cartExist) {
                            ...
                            $cart['food_addons'] = $orderProduct->food_addons ?? json_encode([]);
                            $cart['food_addon_total'] = $orderProduct->food_addon_total ?? 0;
                            ...
                            $cart['is_checked'] = 1;
                            $cart['discount'] = Helpers::getProductDiscount($product, $basePrice);
