...
class CartManager
{
        ...
        if (session()->has('guest_id') || (!is_null($request) && !is_null($request->guest_id)) ) {
            ...
            foreach ($cartList as $cart) {
                ...
                Cart::where([
                    ...
                    'food_addons' => $cart['food_addons'] ?? json_encode([]),
                    'seller_id' => $cart['seller_id'],
                    'seller_is' => $cart['seller_is']
                ])->delete();
    ...
    public static function getCartListQuery($groupId = null, $type = null)
    {
        return Cart::with(['product' => function ($query) {
            ...
            ->get()?->each(function ($item) {
                $basePrice = max(0, $item['price'] - ($item['food_addon_total'] ?? 0));
    ...
    public static function addToCartPhysicalProduct($request, $product, $shippingType, $sellerShippingList): array
    {
        ...
        $string = '';
        $variations = [];
        $foodAddonData = self::getSelectedFoodAddonData(product: $product, request: $request, failOnInvalid: true);

        if ($foodAddonData['status'] === 0) {
            return ['status' => 0, 'message' => $foodAddonData['message']];
        }
        ...
        $cartArray = [
            ...
            'variant' => $string,
            'food_addons' => $foodAddonData['food_addons_json'],
            'food_addon_total' => $foodAddonData['food_addon_total'],
        ];
        ...
        $basePrice = $price;
        $tax = Helpers::tax_calculation(product: $product, price: $basePrice, tax: $product['tax'], tax_type: 'percent');
        $getProductDiscount = getProductPriceByType(product: $product, type: 'discounted_amount', result: 'value', price: $basePrice);
        $price = $basePrice + $foodAddonData['food_addon_total'];
    ...
    public static function getSelectedFoodAddonData(Product $product, $request, bool $failOnInvalid = false): array
    {
        $emptyResponse = [
            'status' => 1,
            'message' => null,
            'food_addons' => [],
            'food_addons_json' => json_encode([]),
            'food_addon_total' => 0,
        ];

        if (!$product->is_food || empty($product->food_addons) || $product->product_type === 'digital') {
            return $emptyResponse;
        }

        $selectedFoodAddons = $request->input('selected_food_addons', []);
        if (!is_array($selectedFoodAddons)) {
            $selectedFoodAddons = [];
        }
        ...
        foreach ($product->food_addons as $groupIndex => $addonGroup) {
            $options = array_values($addonGroup['options'] ?? []);
            if (empty($options)) {
                continue;
            }

            $selectedIndexes = array_values(array_unique(array_map('intval', array_filter(Arr::wrap($selectedFoodAddons[$groupIndex] ?? []), function ($value) {
                return $value !== null && $value !== '';
            }))));

            $isRequired = (bool)($addonGroup['is_required'] ?? false);
            $minimumSelection = max(0, (int)($addonGroup['min_select'] ?? 0));
            $maximumSelection = min(max(1, (int)($addonGroup['max_select'] ?? count($options))), count($options));
            if ($isRequired && $minimumSelection === 0) {
                $minimumSelection = 1;
            }

            if (count($selectedIndexes) > $maximumSelection) {
                return $failOnInvalid ? [
                    'status' => 0,
                    'message' => 'You can not select more than ' . $maximumSelection . ' add-ons in ' . ($addonGroup['name'] ?? 'this group') . '.',
                ] : $emptyResponse;
            }

            if (count($selectedIndexes) < $minimumSelection) {
                return $failOnInvalid ? [
                    'status' => 0,
                    'message' => 'Please select required add-ons for ' . ($addonGroup['name'] ?? 'this product') . '.',
                ] : $emptyResponse;
            }
            ...
            $preparedOptions = [];
            foreach ($selectedIndexes as $optionIndex) {
                if (!array_key_exists($optionIndex, $options)) {
                    return $failOnInvalid ? [
                        'status' => 0,
                        'message' => 'Invalid food add-on selection provided.',
                    ] : $emptyResponse;
                }

                $option = $options[$optionIndex];
                $optionPrice = (float)($option['price'] ?? 0);
                $foodAddonTotal += $optionPrice;
                $preparedOptions[] = [
            ...
            $preparedAddons[] = [
                ...
                'name' => $addonGroup['name'] ?? 'Add-on',
                'min_select' => $minimumSelection,
                'max_select' => $maximumSelection,
        ...
        return [
            'status' => 1,
            'message' => null,
            'food_addons' => $preparedAddons,
            'food_addons_json' => json_encode($preparedAddons),
            'food_addon_total' => $foodAddonTotal,
        ];
    }
}
