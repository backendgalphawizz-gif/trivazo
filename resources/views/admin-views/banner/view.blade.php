...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        <div class="card mb-3 border-warning">
            <div class="card-body">
                <h5 class="mb-3">PHP Upload Debug</h5>
                <div class="row g-2">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        <strong>max_input_time:</strong> {{ ini_get('max_input_time') }}
                    </div>
                    <div class="col-md-4">
        ...
        <div class="d-flex justify-content-between align-items-center gap-3 mb-3">
            ...
            <div class="btn-group">
                <div class="ripple-animation d-none" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        <label for="banner_category_select" class="title-color text-capitalize">
                                            Banner Category
                                        </label>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="js-example-responsive form-control w-100" name="banner_type" required id="banner_type_select">
                                            @foreach($allowedBannerTypes as $key => $banner)
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <small class="text-muted d-block mt-2" id="media_type_note">Video is available only for normal main banner.</small>
                                    </div>
                                    ...
                                    <div class="form-group mb-0 d-none" id="resource-product">
                                        <label for="product_id"
                                               class="title-color text-capitalize">{{ translate('food') }}</label>
                                ...
                                <div class="col-md-6 d-flex flex-column justify-content-center">
                                    <div>
                                        ...
                                        <div id="banner-video-wrapper" class="d-none">
                                            <div class="form-group mb-3">
                                                <label for="video" class="title-color text-capitalize">{{ translate('banner_video') }}</label>
                                                <input type="file" name="video" class="form-control-file text--primary font-weight-bold" id="video" accept="video/mp4,video/webm,video/ogg,video/quicktime">
                                            </div>
                        ...
                        <table id="columnSearchDatatable"
                            ...
                            @foreach($banners as $key=>$banner)
                                ...
                                <tr id="data-{{ $banner->id}}">
                                    ...
                                    @php($bannerCategory = strtolower($banner->banner_category ?? ''))
                                    @php($bannerCategoryLabel = $bannerCategory === 'food' ? 'Food' : ($bannerCategory === 'normal' ? 'Normal' : (str_contains(strtolower(($banner->banner_type ?? '')), 'food') ? 'Food' : 'Normal')))
                                    <td>
                                        <span class="badge badge-soft-info text-capitalize">{{ $bannerCategoryLabel }}</span>
                                    </td>
                                    <td>
                                        @php($videoFile = $banner->video ?? null)
                                        @php($photoFile = $banner->photo ?? null)
                                        @php($photoFullUrl = data_get($banner, 'photo_full_url'))
                                        @php($photoSource = data_get($photoFullUrl, 'path')
                                            ? getStorageImages(path: $photoFullUrl, type: 'backend-banner')
                                            : (!empty($photoFile)
                                                ? asset('storage/banner/' . basename(parse_url($photoFile, PHP_URL_PATH) ?: $photoFile))
                                                : null))
                                        @php($videoExtensions = ['mp4', 'webm', 'ogg', 'mov'])
                                        @php($photoExtension = $photoFile ? strtolower(pathinfo($photoFile, PATHINFO_EXTENSION)) : null)
                                        @php($isVideoBanner = !empty($videoFile) || in_array($photoExtension, $videoExtensions))
                                        @php($videoSource = !empty($videoFile)
                                            ? $banner->video_full_url
                                            : (!empty($photoFile) && in_array($photoExtension, $videoExtensions)
                                                ? asset('storage/banner/' . basename(parse_url($photoFile, PHP_URL_PATH) ?: $photoFile))
                                                : null))
                                        @php($videoExtension = !empty($videoFile)
                                            ? strtolower(pathinfo(basename(parse_url($videoFile, PHP_URL_PATH) ?: $videoFile), PATHINFO_EXTENSION))
                                            : $photoExtension)

                                        @if($isVideoBanner && $videoSource)
                                            <video class="rounded" width="120" height="70" controls preload="metadata" muted>
                                                <source src="{{ $videoSource }}" type="video/{{ $videoExtension }}">
                                        ...
                                        <form action="{{ route('admin.banner.status') }}" method="post" id="banner-status{{ $banner['id'] }}-form">
                                            ...
                                            <label class="switcher">
                                                <input type="checkbox" class="switcher_input toggle-switch-message" name="status"
                                                       ...
                                                       data-on-message="<p>{{ translate('if_enabled_this_banner_will_be_available_on_the_customer_app') }}</p>"
                                                       data-off-message="<p>{{ translate('if_disabled_this_banner_will_be_hidden_from_the_customer_app') }}</p>">
...
@push('script')
    ...
    <script>
        ...
        function getThemeWiseRatio(){
            ...
            let get_ratio = theme_ratio[theme] && theme_ratio[theme][banner_type] ? theme_ratio[theme][banner_type] : '4:1';
        ...
        function syncBannerRestrictions() {
            ...
            let mediaTypeGroup = elementMediaTypeSelect.closest('.form-group');

            mediaTypeGroup.removeClass('d-none');
