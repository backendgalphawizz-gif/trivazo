...
@section('content')
    <div class="content container-fluid text-start">
            ...
            <div class="card-body">
                ...
                <div class="d-flex gap-3 flex-wrap">
                    ...
                    <div class="row gy-3 flex-grow-1">
                        ...
                        @if($product->is_food)
                            <div class="col-sm-6 col-xl-4">
                                <h4 class="mb-3 text-capitalize">Food information</h4>
                                <div class="pair-list">
                                    <div>
                                        <span class="key text-nowrap">Food type</span>
                                        <span>:</span>
                                        <span class="value">{{ ucwords(str_replace('_', ' ', $product->food_type ?? '-')) }}</span>
                                    </div>
                                    <div>
                                        <span class="key text-nowrap">Preparation time</span>
                                        <span>:</span>
                                        <span class="value">{{ $product->prep_time ? $product->prep_time.' min' : '-' }}</span>
                                    </div>
                                    <div>
                                        <span class="key text-nowrap">Available time</span>
                                        <span>:</span>
                                        <span class="value">
                                            {{ $product->available_from ? substr((string) $product->available_from, 0, 5) : '--:--' }}
                                            -
                                            {{ $product->available_to ? substr((string) $product->available_to, 0, 5) : '--:--' }}
                                        </span>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="row g-2 mt-3">
            @if($product->is_food && !empty($product->food_addons))
                <div class="col-md-12">
                    <div class="card border-0 h-100">
                        <div class="card-header bg--primary--light">
                            <h5 class="card-title text-capitalize mb-0">Food add-ons</h5>
                        </div>
                        <div class="card-body">
                            <div class="row g-3">
                                @foreach($product->food_addons as $addonGroup)
                                    <div class="col-md-6 col-xl-4">
                                        <div class="border rounded p-3 h-100">
                                            <div class="d-flex align-items-center justify-content-between gap-2 mb-2">
                                                <h5 class="mb-0">{{ $addonGroup['name'] ?? 'Addon Group' }}</h5>
                                                @if(!empty($addonGroup['is_required']))
                                                    <span class="badge badge-soft-warning">Required</span>
                                                @endif
                                            </div>
                                            <p class="text-muted fz-12 mb-3">
                                                Min {{ $addonGroup['min_select'] ?? 0 }} / Max {{ $addonGroup['max_select'] ?? 0 }}
                                            </p>
                                                ...
                                                <table class="table table-sm table-borderless mb-0">
                                                    ...
                                                    <tbody>
                                                        @foreach(($addonGroup['options'] ?? []) as $option)
                                                            <tr>
                                                                <td>{{ $option['name'] ?? '-' }}</td>
                                                                <td class="text-right">{{ setCurrencySymbol(amount: $option['price'] ?? 0, currencyCode: getCurrencyCode()) }}</td>
                                                            </tr>
                                                        @endforeach
                                                    </tbody>
                                                </table>
