    ...
    <form class="user" action="{{route('admin.vendors.add')}}" method="post" enctype="multipart/form-data" id="add-vendor-form">
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2 border-bottom pb-3 mb-4 pl-4">
                    ...
                    {{ translate('vendor_type_selection') }}
                </h5>
                <div class="row">
                    <div class="col-lg-12">
                        <div class="form-group">
                            <label class="title-color d-flex gap-1 align-items-center">{{translate('vendor_type')}} <span class="text-danger">*</span></label>
                            <div class="d-flex gap-4">
                                <div class="custom-control custom-radio">
                                    <input type="radio" id="vendor_type_food" name="vendor_type" value="food" class="custom-control-input vendor-type-radio" checked required>
                                    <label class="custom-control-label" for="vendor_type_food">{{translate('food_vendor')}}</label>
                                </div>
                                <div class="custom-control custom-radio">
                                    <input type="radio" id="vendor_type_hotel" name="vendor_type" value="hotel" class="custom-control-input vendor-type-radio" required>
                                    <label class="custom-control-label" for="vendor_type_hotel">{{translate('hotel_vendor')}}</label>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card mt-3">
            <div class="card-body">
                <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2 border-bottom pb-3 mb-4 pl-4">
                    <img src="{{dynamicAsset(path: 'public/assets/back-end/img/vendor-information.png')}}" class="mb-1" alt="">
                    {{translate('bussiness_information')}}
                </h5>

                <div class="row">
                    <div class="col-lg-6 form-group">
                        <label for="shop_name" class="title-color d-flex gap-1 align-items-center">{{translate('bussiness_name')}}</label>
                    ...
                    <div class="col-lg-6 form-group">
                        <label for="shop_address" class="title-color d-flex gap-1 align-items-center">{{translate('bussiness_address')}}</label>
                    ...
                    <div class="col-12 mt-4 d-none">
                        <div class="border-top pt-4">
                            <h5 class="mb-3">{{translate('commission_settings')}}</h5>
                            
                            <div id="commission_fields" class="vendor-commission-fields">
                                <div class="row">
                                    
                            <div class="col-lg-6 form-group">
                                        <label class="title-color">{{translate('commission_type')}}</label>
                                        <select name="commission_type" class="form-control commission-type">
                                            <option value="percentage">{{translate('percentage')}}</option>
                                            <option value="fixed">{{translate('fixed')}}</option>
                                        </select>
                                    </div>
                            
                                    <div class="col-lg-6 form-group fixed-rate-field">
                                        <label class="title-color">{{translate('fixed_rate')}} (₹)</label>
                                        <input type="number"
                                               name="commission_fixed_rate"
                                               class="form-control"
                                               step="0.01"
                                               min="0"
                                               value="{{ old('commission_fixed_rate', 20) }}"
                                               placeholder="{{translate('enter_fixed_rate')}}">
                                    </div>
                            
                                    <div class="col-lg-6 form-group percentage-field">
                                        <label class="title-color">{{translate('percentage_rate')}} (%)</label>
                                        <input type="number"
                                               name="commission_percentage_rate"
                                               class="form-control"
                                               step="0.01"
                                               min="0"
                                               max="100"
                                               value="{{ old('commission_percentage_rate', 10) }}"
                                               placeholder="{{translate('enter_percentage')}}">
                                    </div>
                                </div>
                            </div>
...
$('.commission-type').on('change', function () {
    if ($(this).val() === 'fixed') {
        $('.fixed-rate-field').show();
        $('.percentage-field').hide();
    } else {
        $('.fixed-rate-field').hide();
        $('.percentage-field').show();
    }
});

// Trigger on page load
$('.commission-type').trigger('change');
