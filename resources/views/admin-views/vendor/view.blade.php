...
@section('title', $seller?->shop->name ?? translate("bussiness_Name"))
                ...
                <ul class="nav nav-tabs flex-wrap page-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link active"
                           href="{{ route('admin.vendors.view', $seller['id']) }}">{{translate('overview')}}</a>
                    </li>
                    @if ($seller['status']!="pending")
                        ...
                        <li class="nav-item">
                            <a class="nav-link"
                               href="{{ route('admin.vendors.view',['id'=>$seller['id'], 'tab'=>'product']) }}">{{translate('food')}}</a>
                        </li>
                            ...
                            @if($seller?->shop->temporary_close || ($seller?->shop->vacation_status && $current_date >= date('Y-m-d', strtotime($seller?->shop->vacation_start_date)) && $current_date <= date('Y-m-d', strtotime($seller?->shop->vacation_end_date))))
                                <div class="d-flex justify-content-between gap-2 mb-4">
                                    @if($seller->shop->temporary_close)
                                        <div class="btn btn-soft-danger">{{translate('this_currently_close_now')}} </div>
                                    @elseif($seller->shop->vacation_status && $current_date >= date('Y-m-d', strtotime($seller->shop->vacation_start_date)) && $current_date <= date('Y-m-d', strtotime($seller->shop->vacation_end_date)))
                                        <div class="btn btn-soft-danger">{{translate('this_currently_on_vacation')}} </div>
                                    @endif
                                </div>
                            @endif
                            <div class="d-block">
                                <h2 class="mb-2 pb-1">{{ $seller->shop? $seller->shop->name : translate("bussiness_Name")." : ".translate("update_Please") }}</h2>
                ...
                <div class="d-flex gap-3 flex-wrap flex-lg-nowrap">
                    <div class="border p-3 w-170">
                        <div class="d-flex flex-column mb-1">
                            <h6 class="font-weight-normal">{{translate('total_food')}} :</h6>
                    ...
                    <div class="row gy-3 flex-grow-1 w-100">
                        <div class="col-sm-6 col-xxl-3">
                            <h4 class="mb-3 text-capitalize">{{translate('bussiness_information')}}</h4>

                            <div class="pair-list">
                                <div>
                                    <span class="key text-nowrap">{{translate('bussiness_name')}}</span>
                                    <span>:</span>
